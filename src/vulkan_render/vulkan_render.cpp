// vulkan_render.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
uint32_t getGraphicsQueueFamily(VkPhysicalDevice physicalDevice) {
  ...
  assert("No queue families support graphics, is this compute-only device? ");
  return 0;
}
...
VkSwapchainKHR createSwapchain(VkDevice device, VkSurfaceKHR surface, uint32_t familyIndex, VkFormat format, uint32_t width, uint32_t height) {
  ...
  createInfo.imageFormat = format; // SHORTCUT: some devices only support BGRA
...
VkPipeline createGraphicsPipeline(VkDevice device, VkPipelineCache pipelineCache, VkRenderPass renderPass, VkShaderModule vs, VkShaderModule fs, VkPipelineLayout layout) {
  ...
  stages[0].sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
  stages[0].stage = VK_SHADER_STAGE_VERTEX_BIT;
  stages[0].module = vs;
  stages[0].pName = "main";
  stages[1].sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
...
int main() {
  ...
  VK_CHECK(vkGetSwapchainImagesKHR(device, swapchain, &swapchainImageCount, 0)); // TODO: workaround for Intel graphics bug: clean this up
